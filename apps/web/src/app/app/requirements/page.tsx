...
export default function RequirementsPage() {
  // Use "default-project" for now since auth/projects aren't implemented yet
  const { data, isLoading, error } = useRequirements("default-project")
        ...
        <DataList>
          ...
          {!isLoading && !error && data?.items.length === 0 && (
            <div className="flex flex-col items-center text-center py-16 text-muted-foreground">
              ...
              <p className="text-[12px]">Create your first requirement to start an impact analysis.</p>
            </div>
          )}
          ...
          {data?.items.map(req => {
            ...
            return (
              <DataListRow key={req.id} gridCols={gridCols} href={`/app/requirements/${req.id}`}>
                <DataListCell>
