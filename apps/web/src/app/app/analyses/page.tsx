...
export default function AnalysesPage() {
  const { data, isLoading, error } = useAnalyses("default-project")
        ...
        <DataList>
          ...
          {!isLoading && !error && data?.items.length === 0 && (
            <div className="flex flex-col items-center text-center py-16 text-muted-foreground">
              ...
              <p className="text-[12px]">Start an impact analysis to see results here.</p>
            </div>
          )}

          {data?.items.map(analysis => {
            const badge = STATUS_BADGE[analysis.status] || STATUS_BADGE.QUEUED
            const isClickable = analysis.status !== "RUNNING" && analysis.status !== "FAILED"
            return (
              <DataListRow
                key={analysis.id}
                gridCols={gridCols}
                href={isClickable ? `/app/analyses/${analysis.id}` : undefined}
              >
                <DataListCell>
                  <div className="font-medium text-[13px] text-foreground">{analysis.requirementRevisionTitle}</div>
                ...
                <DataListCell>
                  <span className={`px-2 py-0.5 border rounded-md text-[10px] font-semibold tracking-wide uppercase ${badge.className}`}>
                    {badge.label}
                  </span>
                </DataListCell>
                <DataListCell>
