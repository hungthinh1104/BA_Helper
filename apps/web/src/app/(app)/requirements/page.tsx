"use client"

import { useMemo } from "react"
import { WorkspacePageHeader } from "@/components/workspace/shared/page-header"
import { AnalystSummaryBand } from "@/components/workspace/shared/analyst-summary-band"
import { useCurrentWorkspace } from "@/lib/project-context"
import { canCreateRequirement } from "@/lib/permissions"
import { DataList, DataListHeader, DataListRow, DataListCell } from "@/components/workspace/shared/data-list"
import { NewRequirementDialog, ReadinessStatusBadge } from "@/components/workspace/requirement/new-requirement-dialog"
import { Button } from "@/components/ui/button"
import { Skeleton } from "@/components/ui/skeleton"
import { useRequirements } from "@/hooks/api/use-requirements"
import { Plus, AlertCircle, CheckCircle2, Clock } from "lucide-react"
import { useLocale, useTranslations } from "next-intl"
import { useLocalizedHref } from "@/i18n/navigation"

const gridCols = "minmax(200px, 2.5fr) minmax(100px, 1fr) 180px 110px"

export default function RequirementsPage() {
  const t = useTranslations("workspaceLists")
  const locale = useLocale()
  const href = useLocalizedHref()
  const { data, isLoading, error } = useRequirements()
  const workspace = useCurrentWorkspace()
  const canCreateReq = workspace ? canCreateRequirement(workspace.membershipRole) : false
  const requirements = useMemo(() => data?.items ?? [], [data?.items])
  const summary = useMemo(() => {
    return requirements.reduce(
      (acc, item) => {
        const status = item.latestRevision.readinessStatus
        if (status === "READY_FOR_ANALYSIS") acc.ready += 1
        if (status === "NEEDS_CLARIFICATION") acc.needsClarification += 1
        if (status === "DRAFT") acc.draft += 1
        acc.validationIssues += item.latestRevision.validationIssues.length
        return acc
      },
      {
        ready: 0,
        needsClarification: 0,
        draft: 0,
        validationIssues: 0,
      },
    )
  }, [requirements])

  return (
      <div className="app-page-scroll">
        <div className="max-w-4xl mx-auto w-full py-4">
        <WorkspacePageHeader
          title={t("requirementsTitle")}
          description={t("requirementsDescription")}
        >
          <NewRequirementDialog>
            <Button size="sm" className="h-8 shadow-none gap-1.5" disabled={!canCreateReq} title={!canCreateReq ? t("analystRequiredRequirement") : undefined}>
              <Plus className="w-3.5 h-3.5" /> {t("newRequirement")}
            </Button>
          </NewRequirementDialog>
        </WorkspacePageHeader>

        <AnalystSummaryBand
          title={t("requirementPipeline")}
          description={
            summary.ready > 0
              ? t("requirementPipelineReadyDescription", { count: summary.ready })
              : summary.needsClarification > 0
                ? t("requirementPipelineClarifyDescription", { count: summary.needsClarification })
                : t("requirementPipelineEmptyDescription")
          }
          items={[
            {
              label: t("ready"),
              value: isLoading ? "..." : summary.ready,
              description: t("readyRequirementsMetric"),
              icon: <CheckCircle2 className="h-4 w-4 text-success" />,
            },
            {
              label: t("needsClarificationShort"),
              value: isLoading ? "..." : summary.needsClarification,
              description: t("clarificationMetric"),
              icon: <AlertCircle className="h-4 w-4 text-warning" />,
            },
            {
              label: t("draft"),
              value: isLoading ? "..." : summary.draft,
              description: t("draftMetric"),
              icon: <Clock className="h-4 w-4" />,
            },
            {
              label: t("validationIssues"),
              value: isLoading ? "..." : summary.validationIssues,
              description: t("validationIssuesMetric"),
              icon: <AlertCircle className="h-4 w-4 text-destructive" />,
            },
          ]}
        />

        <DataList>
          <DataListHeader gridCols={gridCols}>
            <DataListCell>{t("title")}</DataListCell>
            <DataListCell>{t("revisionId")}</DataListCell>
            <DataListCell>{t("readiness")}</DataListCell>
            <DataListCell>{t("created")}</DataListCell>
          </DataListHeader>

          {isLoading && (
            <>
              {[1, 2, 3].map((i) => (
                <DataListRow key={i} gridCols={gridCols}>
                  <DataListCell><Skeleton className="h-4 w-[200px]" /></DataListCell>
                  <DataListCell><Skeleton className="h-4 w-[80px]" /></DataListCell>
                  <DataListCell><Skeleton className="h-5 w-[100px] rounded-full" /></DataListCell>
                  <DataListCell><Skeleton className="h-4 w-[60px]" /></DataListCell>
                </DataListRow>
              ))}
            </>
          )}

          {error && (
            <div className="flex flex-col items-center py-16 text-muted-foreground">
              <AlertCircle className="w-6 h-6 text-destructive mb-4" />
              <p className="text-[13px] font-medium text-foreground">{t("failedToLoadRequirements")}</p>
              <p className="text-[12px]">{error.message}</p>
            </div>
          )}

          {!isLoading && !error && data?.items.length === 0 && (
            <div className="flex flex-col items-center text-center py-16 text-muted-foreground">
              <div className="w-12 h-12 rounded-lg bg-surface border border-border/50 flex items-center justify-center mb-4">
                <Plus className="w-5 h-5" />
              </div>
              <p className="text-[13px] font-medium text-foreground mb-1">{t("noRequirements")}</p>
              <p className="text-[12px] mb-4">{t("noRequirementsDescription")}</p>
              <NewRequirementDialog>
                <Button size="sm" variant="outline" className="h-8 shadow-none gap-1.5" disabled={!canCreateReq} title={!canCreateReq ? t("analystRequiredRequirement") : undefined}>
                  <Plus className="w-3.5 h-3.5" /> {t("newRequirement")}
                </Button>
              </NewRequirementDialog>
            </div>
          )}

          {data?.items.map(req => {
            const latestRev = req.latestRevision
            return (
              <DataListRow key={req.id} gridCols={gridCols} href={href(`/requirements/${req.id}`)}>
                <DataListCell>
                  <div className="flex flex-col gap-0.5">
                    <span className="text-[13px] font-medium text-foreground leading-snug">{latestRev.title}</span>
                    <span className="text-[11px] text-muted-foreground line-clamp-1">
                      {latestRev.rawText}
                    </span>
                    {latestRev.validationIssues.length > 0 && (
                      <span className="text-[10px] text-destructive font-medium flex items-center gap-1">
                        <AlertCircle className="w-3 h-3" /> {t("issues", { count: latestRev.validationIssues.length })}
                      </span>
                    )}
                  </div>
                </DataListCell>
                <DataListCell>
                  <span className="font-mono text-[11px] text-muted-foreground/80">{latestRev.id.split("-")[0]}</span>
                </DataListCell>
                <DataListCell>
                  <ReadinessStatusBadge status={latestRev.readinessStatus} />
                </DataListCell>
                <DataListCell>
                  <span className="text-[12px] text-muted-foreground">
                    {new Date(latestRev.createdAt).toLocaleDateString(locale, { month: "short", day: "numeric" })}
                  </span>
                </DataListCell>
              </DataListRow>
            )
          })}
        </DataList>
        </div>
      </div>
    )
}
