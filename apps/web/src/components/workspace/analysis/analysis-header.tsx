    ...
    <div className="space-y-4">
      {isStale ? (
        <div className="flex items-start gap-3 rounded-xl border border-warning/30 bg-warning/10 px-4 py-3 text-sm text-warning">
          ...
          <div className="space-y-1">
            <p className="font-medium">{t("snapshotIsStale")}</p>
            <p className="leading-6">
              {t("snapshotStaleDescription")}
            </p>
          </div>
        </div>
      ) : null}
      ...
      {finalized ? (
        <div className="flex items-start gap-3 rounded-xl border border-success/25 bg-success/10 px-4 py-3 text-sm text-success">
          ...
          <div className="space-y-1">
            <p className="font-medium">{t("analysisFinalized")}</p>
            <p className="leading-6">
              {t("analysisFinalizedDescription")}
            </p>
          </div>
        </div>
      ) : null}
      ...
      <WorkspacePageHeader
        title={analysis.requirement.revisionTitle || analysis.requirement.id || t("currentRequirementChange")}
        description={
          <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
            <span className="inline-flex items-center gap-1.5">
              <GitBranch className="h-3.5 w-3.5" />
              {t("targetRef", { ref: analysis.sourceTarget.requestedRef })}
            </span>
            <span>{t("commitShort", { commit: analysis.snapshot.commitSha.substring(0, 7) })}</span>
          </div>
        }
        className="mb-0"
      >
      ...
      <div className="grid grid-cols-[repeat(auto-fit,minmax(140px,1fr))] gap-3">
        <MetricCard label={t("coverage")} value={analysis.snapshot.coverageStatus ?? t("unknown")} />
        <MetricCard label={t("reviewRemaining")} value={`${blockingRemaining}`} />
        <MetricCard label={t("riskUnknown")} value={`${stats.unknowns + stats.conflicts}`} />
      </div>
    </div>
  )
}
