...
const RepositoryOption = memo(function RepositoryOption({
  repository,
  selected,
  onToggle,
}: {
  repository: RepositoryListItemResponse
  selected: boolean
  onToggle: (repository: RepositoryListItemResponse) => void
}) {
  const handleToggle = useCallback(() => onToggle(repository), [onToggle, repository])
  const coverage = repository.latestSnapshot?.coverageStatus
    ...
    >
      <div className="flex items-center justify-between gap-3 mb-1">
        <div className="flex items-center gap-2">
          ...
          <p className="text-[13px] font-mono font-medium text-foreground">{repository.displayName}</p>
        </div>
        <span
      ...
      <p className="text-[11px] text-muted-foreground/70 font-mono">
        {repository.canonicalUrl.replace("https://github.com/", "")} · {repository.latestTarget?.requestedRef}
      </p>
    </button>
  )
})
...
export function RepositorySelectionStep({
  ...
  const selectedRepoIds = useMemo(
    () => new Set(selectedRepos.map((repo) => repo.id)),
    [selectedRepos],
  )
      ...
      <div className="px-6 py-4 flex flex-col gap-2 max-h-72 overflow-y-auto">
        {reposLoading ? (
          <div className="py-8 flex flex-col items-center gap-2 text-muted-foreground">
            ...
            <span className="text-[13px]">{t("loadingRepositories")}</span>
          </div>
        ) : reposError ? (
          <div className="py-8 flex flex-col items-center gap-2 text-muted-foreground">
            <AlertCircle className="w-5 h-5 text-danger" />
            <span className="text-[13px]">{t("failedRepositories")}</span>
          </div>
        ) : readyRepos.length === 0 ? (
          <div className="py-8 flex flex-col items-center gap-3 text-center text-[13px] text-muted-foreground">
            <p>{t("noIndexedRepositories")}</p>
            <ConnectRepoDialog>
      ...
      <div className="px-6 py-4 border-t border-border/60 bg-surface-muted/30 flex justify-between gap-2">
        ...
        <div className={`flex items-center gap-3 ${hasPreselectedReq ? "w-full justify-between" : ""}`}>
          <span className="text-[11px] text-muted-foreground">
            {t("repositoriesSelected", { count: selectedRepos.length })}
          </span>
          <Button
            size="sm"
