    ...
    <main className="flex min-w-0 flex-1 flex-col bg-surface overflow-hidden relative">
      <div className="flex-1 overflow-y-auto p-4 lg:p-6">
        <div className="mx-auto max-w-2xl">
          ...
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-[11px] text-muted-foreground">{activeItemIndex + 1} of {totalItems}</span>
          </div>
          ...
          {/* Why it matters */}
          <div className="mb-5">
            <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground mb-2">Why it matters</p>
          ...
          {activeItem.suggestedAction && (
            <div className="mb-6">
              <p className="text-[11px] font-semibold uppercase tracking-wider text-info mb-2">Suggested Action</p>
      ...
      <div className="absolute bottom-0 left-0 right-0 z-10 flex flex-col items-center justify-between gap-3 border-t border-border bg-background px-4 pt-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))] shadow-sm sm:flex-row sm:gap-0 lg:px-6">
        <div className="flex items-center gap-2 w-full sm:w-auto justify-between sm:justify-start">
          ...
          <span className="text-[11px] text-muted-foreground sm:hidden">{activeItemIndex + 1} of {totalItems}</span>
          <Button
            variant="outline"
            size="sm"
            onClick={onNext}
        ...
        <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto justify-between sm:justify-end">
          <span className="text-[11px] text-muted-foreground mr-2 hidden sm:inline">{activeItemIndex + 1} of {totalItems}</span>
          ...
          {activeItem.reviewStatus === "CONFIRMED" || activeItem.reviewStatus === "REJECTED" ? (
            ...
            <span className="text-[12px] font-medium text-muted-foreground flex items-center gap-1.5">
              <AlertCircle className="w-3.5 h-3.5" /> No Decision Required
            </span>
          ) : !canReview ? (
            <span className="text-[12px] font-medium text-warning">Read-only Review</span>
          ) : (
            <div className="flex items-center gap-2">
              <TooltipProvider>
                <Tooltip>
                  ...
                  <TooltipContent>
                    <p>Skip in this session. Status unchanged.</p>
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
