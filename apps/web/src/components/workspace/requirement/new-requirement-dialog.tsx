...
export function NewRequirementDialog({ children, existingRequirement }: NewRequirementDialogProps) {
  ...
  const latestRev = useMemo(() => {
    return existingRequirement && existingRequirement.revisions.length > 0
      ? existingRequirement.revisions.reduce(
          (latest, revision) =>
            revision.versionNumber > latest.versionNumber ? revision : latest,
          existingRequirement.revisions[0],
        )
      : null
  }, [existingRequirement])
  ...
  const handleSubmit = async () => {
    try {
      ...
      toast.success(t("requirementSaved"))
      setResult(req)
      setState("result")
    } catch (err: unknown) {
      toast.error(t("failedSaveRequirement"), {
  ...
  const parseResult = useMemo(
    () =>
      requirementCreateRequestSchema.safeParse({
        title: title.trim(),
        rawText: rawText.trim(),
        submitForReadinessCheck: submitForCheck,
      }),
    [rawText, submitForCheck, title],
  )
        ...
        <DialogHeader className="px-6 pt-5 pb-4 border-b border-border/60">
          <div className="flex items-center justify-between">
            <DialogTitle className="text-[15px]">
              {state === "form" 
                ? (existingRequirement ? t("editRequestNewRevision") : t("newRequirement"))
                : t("readinessCheckResult")}
            </DialogTitle>
          ...
          <div className="px-6 py-5 flex flex-col gap-5">
            {existingRequirement && (
              <div className="flex items-start gap-2.5 p-3 rounded-lg bg-surface-muted border border-border/50 text-[12px] text-muted-foreground leading-relaxed">
                ...
                {t("editingCreatesRevision")}
              </div>
            )}

            {/* Title */}
            <div className="flex flex-col gap-1.5">
              <label className="text-[12px] font-semibold text-muted-foreground uppercase tracking-wider">{t("title")}</label>
              <input
            ...
            <div className="flex flex-col gap-1.5">
              <div className="flex items-center justify-between">
                <label className="text-[12px] font-semibold text-muted-foreground uppercase tracking-wider">{t("changeRequestText")}</label>
            ...
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              <span className="text-[13px] text-foreground/80">{t("submitForReadinessCheck")}</span>
            </label>
            ...
            <div className="-mx-6 px-6 py-4 border-t border-border/60 bg-surface-muted/30 flex justify-end gap-2">
              ...
              <Button size="sm" className="h-8 shadow-none" disabled={!canSubmit || loading} onClick={handleSubmit}>
                {loading ? t("checking") : submitForCheck ? t("submitCheckReadiness") : t("saveAsDraft")}
              </Button>
            </div>
          </div>
        )}
          ...
          <div className="px-6 py-5 flex flex-col gap-5">
            ...
            <div className="-mx-6 px-6 py-4 mt-2 border-t border-border/60 bg-surface-muted/30 flex justify-end gap-2">
              <Button size="sm" variant="outline" className="h-8 shadow-none" onClick={reset}>
                {result.readinessStatus === "NEEDS_CLARIFICATION" ? t("editRequirement") : t("startOver")}
              </Button>
              <Button size="sm" className="h-8 shadow-none" onClick={handleAccept}>
                {result.readinessStatus === "NEEDS_CLARIFICATION" ? t("saveAnyway") : existingRequirement ? t("saveRevision") : t("addToWorkspace")}
              </Button>
            </div>
          </div>
        )}
...
function ReadinessStatusBadge({ status }: { status: RequirementCreateResponse["readinessStatus"] }) {
  ...
  const map: Record<RequirementCreateResponse["readinessStatus"], { label: string; className: string }> = {
    READY_FOR_ANALYSIS: { label: t("readyForAnalysis"), className: "bg-success/10 text-success border-success/50" },
    NEEDS_CLARIFICATION: { label: t("needsClarification"), className: "bg-destructive/10 text-destructive border-destructive/50" },
    DRAFT: { label: t("draft"), className: "bg-surface-muted text-muted-foreground border-border" },
    ARCHIVED: { label: t("archived"), className: "bg-surface-muted text-muted-foreground border-border" },
  }
