...
export class AnswerClarificationUseCase {
  ...
  async execute(id: string, answer: string) {
    ...
    if (clarification.status !== 'OPEN') {
      throw new AppError(
        ...
        'Can only answer OPEN clarifications',
      );
    }
    ...
    if (analysis.status === 'COMPLETED') {
      throw new AppError(
        ...
        'Cannot answer clarification for a completed analysis',
      );
    }

    return this.clarificationRepo.updateStatusAndAnswer(id, 'ANSWERED', answer);
  }
}
