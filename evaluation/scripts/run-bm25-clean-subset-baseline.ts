...
import { rankBm25Artifacts } from '../baselines/bm25-baseline';
...
function main() {
  ...
  if (subsetParam !== 'clean-vector-ready-v0') {
    console.error('Usage: run-bm25-clean-subset-baseline.ts --subset clean-vector-ready-v0');
    process.exit(1);
  }

  const runId = `bm25-clean-subset-baseline-v0:${new Date().toISOString().replace(/[:.]/g, '-')}`;
  ...
  try {
    ...
    for (const caseId of subsetData.caseIds) {
      ...
      const topKRaw = rankBm25Artifacts({ evaluationCase, topK: 20 });
    ...
    writeResult({
      canonicalJsonPath: EvaluationPaths.resultsV0.baselines + '/bm25-clean-subset-baseline.v0.json',
      canonicalMarkdownPath: EvaluationPaths.resultsV0.baselines + '/bm25-clean-subset-baseline.v0.md',
      runId,
      relativeArtifactPath: 'baselines/bm25-clean-subset-baseline.v0.json',
      jsonData: artifactJson,
      markdownData: mdLines.join('\n')
    });

    updateManifest(runId);
    console.log(`Successfully completed BM25 baseline on ${caseCount} cases.`);

  } catch (error) {
    console.error('[ERROR] bm25 baseline failed: ', error instanceof Error ? error.message : error);
    process.exit(1);
  }
}
...
function updateManifest(runId: string) {
  ...
  if (existsSync(manifestPath)) {
    ...
    manifest.canonicalArtifacts.bm25CleanSubsetBaseline = 'evaluation/results/v0/baselines/bm25-clean-subset-baseline.v0.json';
