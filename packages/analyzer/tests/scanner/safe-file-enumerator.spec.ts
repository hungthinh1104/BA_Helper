...
describe('SafeFileEnumerator', () => {
  ...
  it('enforces MAX_FILE_COUNT and MAX_TS_FILE_COUNT', async () => {
    ...
    expect(result.diagnostics.some(d => d.code === 'REPO_FILE_LIMIT_EXCEEDED' as any)).toBe(true);
  });
  ...
  it('enforces MAX_REPO_SIZE_MB across enumerated files', async () => {
    ...
    expect(result.isPartial).toBe(true);
    expect(result.diagnostics.some(d => d.code === 'REPO_SIZE_LIMIT_EXCEEDED' as any)).toBe(true);
  });
});
